...
function getUserMessageStorageKey(user) {
  const identity = user?._id || user?.id || user?.email;
  return identity ? `${MESSAGE_STORAGE_KEY}:${encodeURIComponent(identity)}` : null;
}
...
export default function AiAssistant() {
  ...
  useEffect(() => {
    ...
    const storageKey = getUserMessageStorageKey(user);
    activeStorageKeyRef.current = storageKey;

    try {
      const stored = JSON.parse(window.localStorage.getItem(storageKey) || '[]');
      setMessages(Array.isArray(stored)
  ...
  useEffect(() => {
    ...
    const storageKey = getUserMessageStorageKey(user);
    if (!storageKey || activeStorageKeyRef.current !== storageKey) return;
  ...
  return <div className="fixed bottom-7 right-7 z-[60] sm:bottom-8 sm:right-8">
    {open ? <section className="hard-card mb-3 w-[min(380px,calc(100vw-2rem))] rounded-[1.5rem] p-4 shadow-2xl">
      ...
      <div className="soft-card promptgrid-scrollbar mb-4 flex min-h-56 max-h-80 flex-col gap-3 overflow-y-auto rounded-2xl p-4 text-sm leading-6" ref={chatRef}>
        {messages.map((item, index) => <div className={`max-w-[88%] break-words rounded-2xl px-3 py-2 leading-6 ${item.role === 'user' ? 'self-end bg-[var(--lime)] text-slate-950' : 'assistant-message self-start'}`} style={item.role === 'assistant' ? { height: 'auto', maxWidth: '88%' } : undefined} key={`${item.role}-${index}`}><span className={item.role === 'assistant' ? 'assistant-message-content' : undefined}>{item.content}</span></div>)}
