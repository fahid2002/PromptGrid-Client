...
export default function NotificationBell() {
  ...
  const read = async (notification) => { if (!notification.readAt) { await api(`/notifications/${notification._id}/read`, { method: 'PATCH' }); setItems((current) => current.map((item) => item._id === notification._id ? { ...item, readAt: new Date().toISOString() } : item)); setUnread((count) => Math.max(0, count - 1)); } setOpen(false); };
