...
function Dashboard() {
  // Get logged-in user data from auth context
  const { user } = useAuth();

  // Get current dashboard route from dynamic URL params
  ...
  return (
    <section className="px-4 py-16 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl">
        ...
        <h1 className="mt-3 font-display text-4xl font-black capitalize sm:text-5xl">
          {user.role} Dashboard
        </h1>
...
function View({ view, data, load, user, page, setPage }) {
  ...
  if (view === 'my-reviews') {
    return <MyReviews reviews={data.reviews} />;
  }
  ...
  if (view === 'admin/payments') {
    return <Payments payments={data.payments} />;
  }
  ...
  if (view === 'admin/analytics') {
    return <Stats items={Object.entries(data)} />;
  }
  ...
  return (
    <div className="soft-card rounded-3xl p-6">
      ...
      <p className="mt-2 muted">
        This dashboard route is not available for your account role.
      </p>
    </div>
  );
}
